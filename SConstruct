...
build_dir_name = "$BUILD_BASE/${ARCH}_%s" % ("debug" if debug else "release")
...
if env.File("$BUILD_BASE/SConscript_conan").exists():
    conan = env.SConscript("$BUILD_BASE/SConscript_conan")
